...
use App\Models\tbl_great_grandparent;
use App\Models\tbl_parent_info;
use Goutte\Client; 
use DB;

class GreatGrandParent  extends Controller
{
	 public function index(Client $client)
     {

      $parentRecord = tbl_parent_info::Parent_records();
          ...
          foreach ($pokemon_row1 as $data) 
          {
              ...
              $Grandfather   = $clientGrandfather->request('GET', 'https://neurotree.org/neurotree/peopleinfo.php?pid='.$id['1']);
                ...
                if ($Grandfather->filter('.boxinline img[src]')->count() > 0 ) 
                  ...
                  $dublicate_check = tbl_great_grandparent::dublicate_check($value->id, $id['1']);
                  ...
                  if(!$dublicate_check){
                   ...
                   $data      = array(
                               'pid'         => trim($id['1']),
                               'parent_info_id' =>trim($value->id), 
                               'grand_parent_name' => $grand_father_name,
                               'address'     => $address,
                 ...
                 if(isset($data))
                   {
                     $lastInsertedID = tbl_great_grandparent::add($data);
                            ...
                            if(strstr($wblink, "http://"))
                              {
                                 ...
                                 $lastInsertedID = tbl_great_grandparent::website_link($lastInsertedID, $wblink);
                                 return true;                                  
                              }
