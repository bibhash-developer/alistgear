            ...
            <div class="col-md-4 col-md-push-4 alistgear_section-card-login">
              <div class="alistgear_section_card_forgetPass">
                ...
                <div class="line_border"></div>
                  ...
                  <div class="card-body">
                    {!! Form::open(['url' =>'resetPassword', 'autocomplete' => 'off']) !!}
                      <div class="form-group row">                        
                        <div class="col-md-12">
                          {!! Form::password('Cpassword',['class' => 'form-control', 'id' => 'email_address', 'placeholder' => 'Change Password']) !!} 
                          {!! Form::hidden('uid', $uid) !!} 
                        </div>
                         <div class="error" id="error-left">
                           @if($errors->has('Cpassword'))
                              <b> {{ $errors->first('Cpassword') }} </b>
                           @endif                          
                        </div>
                      </div>
                       <div class="form-group row">                        
                        <div class="col-md-12">
                          {!! Form::password('Conpassword',['class' => 'form-control', 'id' => 'email_address', 'placeholder' => 'Confirm Password']) !!} 
                        </div>
                         <div class="error" id="error-left">
                           @if($errors->has('Conpassword'))
                              <b> {{ $errors->first('Conpassword') }} </b>
                           @endif                          
                        </div>
                      </div>
